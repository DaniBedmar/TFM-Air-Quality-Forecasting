...
from utils.dictionaries import cities
from utils.functions import get_valid_stations,filter_pollutant

air_path = os.path.join('..','Data','Air_quallity')
files = sorted([f for f in os.listdir(air_path) if os.path.isdir(os.path.join(air_path,f))])
files_path = [os.path.join(air_path,f) for f in files]

for i,dir in enumerate(files_path):
    ...
    valid_stations = get_valid_stations(path_to_meta, cities)
    for j,file in enumerate(files):
        polutant = file.split('_')[0]
        file_path = os.path.join(dir,file)
        
        if j == 0:
            data1 = filter_pollutant(file_path,valid_stations,cities,polutant,0.25)

        elif j == 1:
            data2 = filter_pollutant(file_path,valid_stations,cities,polutant,0.25)

        elif j == 2:
            data3 = filter_pollutant(file_path,valid_stations,cities,polutant,0.25)
